 ...
 * 왜 0개 이상이냐면, 조건에 맞는 행이 하나도 없을 수도 있기 때문이다.
 * 
 * [작성법] 
...
-- * : ALL, 모든, 전부를 뜻한다.
--> EMPLOYEE 테이블에서 '모든' 컬럼을 조회하겠다는 코드로 해석 가능.

-- EMPLOYEE 테이블에서 사번, 직원이름, 휴대전화번호 컬럼만 조회
SELECT EMP_ID, EMP_NAME, PHONE FROM EMPLOYEE; 
-- 만약 컬럼명을 잘못 써 없는 걸로 실행하면 "ORA-00904: "PHONE2": 부적합한 식별자" 로 나온다.

---------------------------------------------------------------------------------------------

-- <컬럼 값 산술 연산>
-- 컬럼값 : 테이블 내 한 칸(== 한 셀)에 작성된 값(즉, DATA 자체)
...
-- ORA-01722: 수치가 부적합합니다 : 타입이 달라 산술 연산이 불가
-- 산술 연산은 숫자 타입(NUMBER 타입)만 가능하다!

SELECT '같음' 
FROM DUAL
WHERE 1 = '1'; -- DB에서는 같다는 의미의 연산자가 =다. ==가 아니다.
...
-- 1(해석 순서 첫 번째). FROM 절 : FROM 테이블명
-- 2. WHERE 절(조건절) : WHERE 컬럼명 연산자 값;
-- 4. ODER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC(오름차순) | DESC(내림차순)] [NULLS FIRST | LAST] 
...
-- 날짜(DATE)에 BETWEEN 이용하기 / 숫자 타입뿐만 아니라 문자 타입도 BETWEEN 사용 가능
-- EMPLOYEE 테이블에서 
...
-- 이메일 '_' 앞이 4글자 이면서
-- 부서코드가 'D9' 또는 'D6' 이고
-- 입사일이 1990-01-01 ~ 2000_12-31이고
-- 급여가 270만원 이상인 사원의
-- 사번, 이름, 이메일, 부서코드, 입사일, 급여 조회
-- Hint! AND가 OR 보다 우선순위가 높으며, ()를 사용하여 우선 순위를 쓸 수 있다.
...
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
WHERE BONUS IS NULL; -- 보너스가 없는 사원들
...
ORDER BY 3 DESC; -- 컬럼 순서 3번째가 SALARY이기에 3을 SALARY 대신 넣어도 상관 없음.

-- 입사일 순서대로 이름, 입사일 조회 (별칭 사용)
SELECT EMP_NAME 이름, HIRE_DATE 입사일
FROM EMPLOYEE
ORDER BY 입사일; -- 해석 순서 때문에 별칭을 ORDER BY 절에 적용시켜 사용할 수 있다. 별칭은 SELECT 절에서 만들 수 있고 해석되는 순서가 3번째고, ORDER BY는 4번째기에 가능한 것이다. *89~92번째 줄 참고
