...
SELECT * FROM DEPARTMENT2; -- SP1 이후의 데이터는 전부 뒤집어 없앴으므로 개발 4팀까지만 남음.

-- ROLLBACK TO "SP1" 구문 수행 시 SP2, SP3인
--SAVEPOINT도 삭제됨.

-- ROLLBACK TO "SP2";
--  'SP2' 저장점이 이 세션에 설정되지 않았거나 부적합합니다. 라고 조회 됨.
...
ROLLBACK TO "SP2";
SELECT * FROM DEPARTMENT2; 
-- 개발 6팀만 삭제(SP2 지점 SAVEPOINT 전 개발5팀 있었으므로)


-- SP1 지점까지 롤백
ROLLBACK TO "SP1";
SELECT * FROM DEPARTMENT2;
-- 개발 5팀 삭제(SP1 지점 SAVEPOINT 전 개발4팀 있었으므로)
