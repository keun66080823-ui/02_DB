...
--			ADD [CONSTRANT 제약조건명] 제약조건(지정할 컬럼명) <-- FK가 아니면 여기까지 작성.
--			[REFERENCES 테이블명[(컬럼명)]]; <-- FK인 경우 추가

-- 2) 삭제 : ALTER TABLE 테이블명 DROP CONSTRAINT 제약조건명;

-- DEPRATMENT 테이블 복사 (컬럼명, 데이터타입, NOT NULL 제약 조건 만 복사가 됨. 나머지 제약 조건은 복사가 안 됨.)
...
-- ORA-12899: "KH_HDK"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
--> DEPT_ID의 데이터 타입이 CHAR(2)이므로 영어+숫자 2글자까지 저장 가능
--> D10은 영어+숫자 총 3글자!!!
--> VARCHAR2(3)으로 변경해보기 (남는 바이트메모리 반환을 위해서 CHAR가 아닌 VARCHAR2로 변경)
...
-- LNAME 의 기본값(DEFAULT 값)을 'KOREA'로 수정
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT 'KOREA';
SELECT * FROM DEPT_COPY;
--> 기본값을 변경했다고 해서 기존 데이터가 변하지는 않음!!!
--> 새로 삽입된 행에서부터 변경된 기본값 적용


-- LNAME '한국' -> 'KOREA' 로 변경하는 UPDATE 문
...
-- DROP TABLE 테이블명 [CASCADE CONSTRAINTS]; -- 부모 자식 관계 테이블임에도 제약 조건을 무시하고 삭제
...
SELECT * FROM TB1;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다 --> TB1이 제대로 삭제됨.
