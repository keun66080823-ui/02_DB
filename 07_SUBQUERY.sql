...
-- 2) 부서코드가 D9인 직원의 이름, 부서코드 조회 (메인 쿼리)
...
/* 서브쿼리 유형
 * 
 * - 단일행 (단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 1개일 때 (행과 열 둘 다 1개)
 * 
 * - 다중행 (단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 여러개일 때 (행이 여러 개)
 * 
 * - 다중열 서브쿼리 : 서브쿼리의 SELECT 절에 나열된 항목수가 여러개일 때 (열이 여러 개)
 * 
 * - 다중행 다중열 서브쿼리 : 조회 결과 행 수와 열 수가 여러개일 때 (행과 열 둘 사 여러 개)
 * 
...
-- 서브쿼리 : 전 직원의 급여 평균
SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE; -- 행과 열이 딱 하나로, 조회 값은 3,047,663 하나 뿐이다.
...
SELECT MAX(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- 7행 1열 : 열은 하나지만 행이 여러 개로 조회됨.
...
-- 2. 선택 함수 사용
--> DECODE(컬럼명, 값1, 값이 1인 경우, 값2, 2인 경우..., 일치하지 않는 경우);
...
-- 1) 직급이 과장인 직원들의 급여 조회 (서브쿼리 : 과장 직급의 최소 급여 = 2200000)
SELECT SALARY
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME = '과장';
...
-- 1) 퇴사한 여직원의 부서코드와 직급코드 조회
SELECT DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE ENT_YN = 'Y'
AND SUBSTR(EMP_NO, 8, 1) = '2'; -- SUBSTR로 EMP_NO<주민등록번호>의 8번째 자리의 하나만 가져올 수 있다. -- D8 J6 (이태림) -- 다중열 서브쿼리
...
-- 3. 77년생 여자 사원과 동일한 부서이면서 동일한 사수를 가지고 있느 사원 조회
...
-- 1) 직급의 평균 급여 (서브쿼리) -- 0으로 넣으면 1의 단위까지 나오고, -4로 넣으면 만원 단위로 나어게 된다.
...
SELECT AVG(SALARY)
FROM EMPLOYEE
WHERE JOB_CODE = 'J3'; -- JOB_CODE에 들어갈 단계는 각 메인쿼리마다 다르기에 JOB_CODE와 비교해야 한다.
...
-- ROWNUM 컬럼 : 행 번호를 나타내는 가상 컬럼 - GUI 없인 행 번호가 조회되지 않기에 만들어졌다.
...
--> 이건 모든 사원에서 급여가 높은 5인을 선별한 게 아니라 
-- 조회 순서 상위 5명끼리의 급여 순위가 정렬되어 조회된 것이다.
-- 해석 순서가 FROM -> WHERE -> SELECT -> ORDER BY임을 잊지 말자.


--> 인라인 뷰를 이용해서 위 문제를 해결할 수 있다.

-- 1) 이름, 급여를 급여 내림차순으로 조회한 결과를 인라인뷰 사용
--> FROM 절에 작성되기 때문에 해석 1순위
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC; -- 이걸 메인 쿼리의 테이블로서 사용
...
WITH TOP_SAL AS (SELECT EMP_NAME, SALARY -- AS 뒤의 괄호 내 코드에 TOP_SAL이라는 이름을 붙임.
FROM EMPLOYEE
ORDER BY SALARY DESC)
SELECT ROWNUM, EMP_NAME, SALARY
FROM TOP_SAL
