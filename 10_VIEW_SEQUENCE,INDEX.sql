...
START WITH 100 -- 시작번호가 100번부터 시작한다는 의미.
INCREMENT BY 5 -- NEXTVAL 호출 시 5씩 증가한다는 의미.
MAXVALUE 150   -- 증가 가능한 최대값이 150이라는 의미.
NOMINVALUE 	   -- 최소값이 없다는 의미.
NOCYCLE		   -- 반복 안함.
...
/* ROWID와 INDEX는 무슨 관계인가?
 * 인덱스가 ROWID를 저장함.
 * 인덱스는 컬럼의 값과 해당 행의 ROWID를 매핑해서 저장함.
 * 즉, 인덱스의 핵심 역할은
 * 컬람값 -> 해당 행의 ROWID를 통해 빠르게 찾아내는 것!
 * */
...
WHERE TEST_ID = 'TEST500000'; -- 0.0s만 나오는데...?

-- 인덱스 0
-- TEST_NO 가 500000인 행 조회
SELECT * FROM TB_IDX_TEST
WHERE TEST_NO = 500000; -- 이것도 마찬가지. 0.0s로 나옴...
-- 보통 10~30배 차이가 남.
