 ...
 * 3 : GROUP BY 그룹을 묶을 컬럼명 (그룹 화)
 * 4 : HAVING 그룹함수식 비교연산자 비교값 (그룹들의 조건)
...
SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용 X (아직 SELECT 해석이 되지 않은 순서이기 때문.)
ORDER BY "인원 수"; -- 별칭 사용 O (SELECT 절 해석 완료된 후기에 가능) 
...
SELECT DEPT_CODE, ROUND(AVG(SALARY)), COUNT(*) -- 부서코드, 평균급여, 인원 수 조회
...
FROM EMPLOYEE
WHERE -- 2000년도 이후의 입사자들을 구하는 여러 조건들
...
-- 결론 : GROUP BY 절에 무엇이 들어가야 할 지 모르겠다면 SELECT 절을 보고 그룹 함수가 아닌 걸 전부 쓰자!
...
FROM EMPLOYEE
--WHERE SALARY >= 3000000 -- WHERE 절에 이렇게 쓰면 한 사람의 급여가 3백만원 이상인 사람들을 걸러서 부서 코드로 나누었다는 뜻이 되버린다. 즉, 요구사항이 다르다.
GROUP BY DEPT_CODE
...
--UNION ALL
MINUS
-- 위와 마찬가지로 EMPLOYEE 테이블에서
-- 급여가 300만 초과인 사원, 사번, 이름, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE SALARY > 3000000;
-- ORA-01789: 질의 블록은 부정확한 수의 결과 열을 가지고 있습니다. // SELECT 문의 개수가 안 맞는다는 뜻
-- ORA-01790: 대응하는 식과 같은 데이터 유형이어야 합니다 // SELECT 문의 개수는 맞췄지만 타입이 다르다는 뜻
