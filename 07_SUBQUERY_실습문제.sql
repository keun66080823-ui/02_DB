...
-- 6. 부서별 입사일이 가장 빠른 사원의					<풀지 못함. 복습 요망>
...
-- DEPT_CODE가 NULL이어서 그런가? 하동운이 안 나옴...
...
WHERE HIRE_DATE = (
					...
					WHERE ENT_YN != 'Y'
					AND MAIN.DEPT_CODE = SUB.DEPT_CODE
					OR (MAIN.DEPT_CODE IS NULL AND SUB.DEPT_CODE IS NULL)) -- 이게 필요했구나...
ORDER BY HIRE_DATE;
					
-- 7. 직급별 나이가 가장 어린 직원의					<풀지 못함. 복습 요망>
...
-- 보너스 포함 연봉
SELECT TO_CHAR((SALARY+SALARY*NVL(BONUS, 0))*12, 'L999,999,999') "보너스 포함 연봉"-- SALARY * ( 1 + NVL(BONUS, 0) ) * 12도 똑같다.
FROM EMPLOYEE;

SYSDATE - TO_DATE(SUBSTR(EMP_N0, 1, 2), 'YY'), (SALARY+SALARY*BONUS)*12
FROM EMPLOYEE MAIN
JOIN JOB USING (JOB_CODE)
