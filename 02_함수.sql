...
-- 단일행 함수 : N개의 값을 읽어서 연산후 N개의 결과를 반환
-- 그룹 함수 : N개의 값을 읽어서 연산 수 1개의 결과를 반환(합계, 평균, 최개, 최소 등)

-- 함수는 SELECT 문의
-- SELECT 절, WHERE 절, ORDER BY절, GROUP BY절, HAVING 절에서 사용 가능

-------------------------- 단일 행 함수 ----------------------------

-- LENGTH(컬럼명 | 문자열) 함수 : 길이 반환
...
------------------------------------------------------------------
-- 소괄호 안의 대괄호는 옵션. 넣어도 그만, 안 넣어도 그만이다.
...
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL; -- 3이 출력됨

-- 문자열을 5번째 문자부터 검색하여 처음으로 검색되는 B 위치 조회
SELECT INSTR('AABAACAABBAA', 'B', 5) FROM DUAL; -- 9가 출력됨
...
-- SURSTR(컬럼명 | 문자열, 잘라내기 시작할 위치 [, 잘라낼 길이])
...
FROM DUAL; -- 'H E L L O' 로 출력됨. 앞 뒤쪽 공백을 제거하되 사이사이 공백은 제거 못함.
...
WHERE ABS(10) = ABS(-10); -- '절대값 같음' 이라고 출력됨. WHERE 절에서도 함수를 작성하는 게 가능.
...
SELECT ROUND(123.456) FROM DUAL; -- 123이 출력
SELECT ROUND(123.456, 1) FROM DUAL; 
-- 123.5가 출력(소수점 두번째 자리에서 반올림하여 소수점 첫번째 자리까지 표기)
...
SELECT ADD_MONTHS(SYSDATE, 4) FROM DUAL; -- 2026-02-20 09:31:10.000
SELECT ADD_MONTHS(SYSDATE, -1) FROM DUAL; -- 2025-09-20 09:30:57.000

-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함
SELECT LAST_DAY(SYSDATE) FROM DUAL; -- 2025-10-31 09:32:29.000
...
-- 0 : 숫자 한 칼을 의미, 여러개 작성 시 오른쪽 정렬 + 빈칸 0 추가
...
-- ORA-01861: 리터럴이 형식 문자열과 일치하지 않음 // 'YYMMDD HH24MISS'이런 식으로 무슨 자리인지 알려줘야 TO_DATE가 알아먹음.
...
-- ORA-01722: 수치가 부적합합니다 // 문자열 안에 콤마가 있으면 완전한 문자열로 인식. 콤마가 없다면 오류가 안 뜨고 1500000라고 나옴
...
/*3*/SELECT SUM(SALARY) "급여 합", ROUND(AVG(SALARY)) "평균"
/*1*/ FROM EMPLOYEE 
...
-- 급여 최대값, 가장늦은 입사일, 알파벳 순서가 가장 늦은 이메일 조회
...
-- 가장 큰 급여 조회
SELECT MAX(SALARY) FROM EMPLOYEE; -- 서브쿼리 + 그룹하무
