...
'991213-1234567', DEFAULT); -- 모든 컬럼에 데이터 다 넣기
...
-- * 컬럼레벨 : 테이블 생성 시 컬럼믕ㄹ 정의하는 부분에 작성하는 것
...
'남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-01400: NULL을 ("KH_HDK"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
INSERT INTO USER_USED_UK VALUES -- 바로 위 코드와 완전히 같은 코드를 하나 더 만들어 수행 시 오류 발생.
(1, 'USER01', 'PASS01', '홍길동', 
'남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_HDK.USER_ID_U)에 위배됩니다
...
(1, NULL, 'PASS01', '홍길동', 
'남자', '010-1234-5678', 'hong@kr.or.kr');
--> 아이디에 NULL 값 중볻 삽입 가능 확인(UNIQUE 제약조건은 실제 데이터값은 중복을 허용하지 않지만, NULL 값은 중복이 허용)
...
CREATE TABLE USER_USED_UK2(
	...
	CONSTRAINT USER_ID_NAME_U UNIQUE(USER_ID, USER_NAME) -- ID나 NAME 둘 중 하나라도 안 겹치면 문제가 없다.
);
...
(1, 'USER02', 'PASS01', '고길동', 
'남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_HDK.USER_ID_NAME_U)에 위배됩니다
...
-- 한 개 컬럼에 설정할 수 있고, 복합키로 설정 가능(PRIMARY 복합키)
...
-- ORA-00001: 무결성 제약 조건(KH_HDK.USER_NO_PK)에 위배됩니다
--> 기본키 중복 오류!

INSERT INTO USER_USED_PK VALUES
(NULL, 'USER02', 'PASS02', '이순신', 
'남자', '010-2222-3333', 'lee@kr.or.kr');
-- ORA-01400: NULL을 ("KH_HDK"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
--> 기본키가 NULL 이므로 오류!
...
(1, 'USER01', 'PASS01', '홍길동', 
'남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_HDK.PK_USERNO_USERID)에 위배됩니다
...
'남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-01400: NULL을 ("KH_HDK"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다

INSERT INTO USER_USED_PK2 VALUES
(3, NULL, 'PASS01', '홍길동', 
'남자', '010-1234-5678', 'hong@kr.or.kr');
 -- ORA-01400: NULL을 ("KH_HDK"."USER_USED_PK2"."USER_ID") 안에 삽입할 수 없습니다
...
--REFERENCES 참조할 테이블명 [(참조할 컬럼명)] [삭제룰]
...
CREATE TABLE USER_GRADE(
	GRADE_CODE NUMBER PRIMARY KEY, -- 등급의 고유식별 번호
...
-- ORA-02291: 무결성 제약조건(KH_HDK.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
--> 외래키 제약조건에 위배되어 오류 발생(부모의 GRADE_CODE에는 50 값 없음)
...
-- ORA-02292: 무결성 제약조건(KH_HDK.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
SELECT * FROM USER_USED_FK2; 
-- 10이었던 자식값이 삭제 옵션으로 인해 NULL로 변함.


-- 3) ON DELETE CASCADE : 부모키 삭제시 자식키도 함께 삭제됨
-- 부모키 삭제 시 값을 사용하고 있던 자식 테이블의 행도 삭제됨.
...
CREATE TABLE USER_USED_FK3(
	...
	EMAIL VARCHAR2(50),
	GRADE_CODE NUMBER CONSTRAINT
	GRADE_CODE_FK3 REFERENCES USER_GRADE3 ON DELETE CASCADE -- 여기가 2번과 다름. 2번은 ON DELETE SET NULL임.
					  					/*삭제 옵션*/
);
...
SELECT * FROM USER_USED_FK3; -- ?? 왜 삭제가 안 되지?????
...
CREATE TABLE USER_USED_CHECK(
	...
	USER_NAME VARCHAR2(30),
	GENDER VARCHAR2(10) CONSTRAINT GENDER_CHECK CHECK(GENDER IN ('남', '여')), -- 컬럼 레벨
	PHONE VARCHAR2(20),
	EMAIL VARCHAR2(50)
	-- 테이블 레벨
...
(1, 'USER01', 'PASS01', '홍길동', 
'남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-02290: 체크 제약조건(KH_HDK.GENDER_CHECK)이 위배되었습니다
